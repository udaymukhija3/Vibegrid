...
| The pivot is on `main` but **NOT deployed** | `b298373` + jsonb fix `fc43adc`, CI green. Deploy workflow run 32587473337 exited 0 via its guard: `RENDER_DEPLOY_HOOK_URL is not set; skipping`. `gh secret list` is empty and `render.yaml:21` sets `autoDeploy: false` | **Both deploy paths are closed by design**, so a green CI run and a green deploy job together still ship nothing. `vibegrid.onrender.com` continues to serve the retired hidden-category game — `/api/puzzles/today` still returns a 16-tile board. A green workflow conclusion is not evidence of a deployment; only the running origin is. | Deploying needs the owner's Render dashboard: create a Deploy Hook and save it as the `RENDER_DEPLOY_HOOK_URL` GitHub secret. Do **not** flip `autoDeploy: true` as a shortcut — that removes the CI gate that just caught `fc43adc`. Confirm with `GET /api/vibes/today` returning 200 on the public origin. |
| `TestPostgresVibeRoundTransactions` ran for the first time on 22 Aug 2026 and **failed** | `invalid input syntax for type json (SQLSTATE 22P02)` in CI | The board insert bound marshalled tiles as `[]byte`, which the driver sends as bytea. It had never executed against a real database because the test skips without `TEST_DATABASE_URL`. Fixed in `fc43adc` by binding `string(tiles)`, matching the idempotency column that failed identically in `7a5b8d4`. **This is the second time this bug class has shipped.** | Partly closed: the serial transactional path is now evidenced by a green CI SHA. **Still missing** is concurrency evidence — no test races two submits or two ballots, so the one-card and one-ballot uniqueness constraints are satisfied serially rather than proven under contention. Treat any new `jsonb` parameter as requiring a Postgres-backed test before merge. |
