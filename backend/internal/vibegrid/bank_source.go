...
type BankPuzzleSource struct {
	inner PuzzleSource
	bank  []Puzzle
	store AdminPuzzleStore
}

// NewBankPuzzleSource wraps inner with bank-backed daily rotation. If the bank is
// empty it behaves exactly like inner.
func NewBankPuzzleSource(inner PuzzleSource, bank []Puzzle, store AdminPuzzleStore) *BankPuzzleSource {
	return &BankPuzzleSource{inner: inner, bank: bank, store: store}
}
...
// EnsureDailyPersisted checks if today's puzzle is missing from the database.
// If missing, it generates a deterministic daily from the bank and persists it,
// ensuring the puzzle has a canonical representation in the database with a
// consistent puzzle sequence number.
func (s *BankPuzzleSource) EnsureDailyPersisted(ctx context.Context, date string) error {
	if s.store == nil {
		return nil
	}
	puzzle, err := s.inner.TodaysPuzzle(ctx, date)
	if err == nil && puzzle.PublishDate == date {
		return nil
	}
	daily, err := s.bankDailyFor(date)
	if err != nil {
		return err
	}
	return s.store.PersistDaily(ctx, daily)
}
